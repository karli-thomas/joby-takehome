...
export class PlanesDashboardComponent implements OnInit {
  ...
  public currentFleetPositions: Array<IAirplanePosition> = [];
  ...
  private startUpdatingFakeData(): void {
    interval(1000).subscribe(_ => { // "_" means the function gets a value but we ignore it.
      ...
      // update currentFleetPos objects so that the location changes are seen as updates
      this.currentFleetPositions = this.currentFleetPositions.map(obj => ({...obj}));
